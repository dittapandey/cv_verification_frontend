...
function DetailCard(props) {
  ...
  return <Card className="detailCard">{point.title}</Card>;
}
...
function Table({ columns, data, renderRowSubComponent }) {
  ...
  return (
    <div className="table_content">
      ...
      >
        <thead
          style={{ backgroundColor: "#343A40", padding: "10px", width: "100%" }}
        >
          ...
          {page.map((row, i) => {
            ...
            return (
              <Fragment {...row.getRowProps()}>
                <tr
                  dataToggle="collapse"
                  dataTarget="#collapseData"
                  className={row.original.status + " accordian-toggle"}
                  style={{
                    height: "50px",
                    marginTop: "10px",
                  }}
...
const ViewPublic = () => {
  ...
  const renderRowSubComponent = useCallback(
    ({ row }) => <span>{JSON.stringify(row.values)}</span>,
    []
  );
